...
* Uses events to automatically to insert, update and delete courseassets
...
class CourseAssetsModule extends AbstractApiModule {
  ...
  */
  ...
  async insertContent(results) {
    ...
    const fileArray = await this.jsonAssets(results);

    if (!fileArray || fileArray.length === 0) return;

    const findAsset = this.assets.find({ path: assetName });

    // TODO find matching assets and increment/insert course asset record
    const finalCourseAssets = {};
  ...
  async deleteContent(results) {
    ...
    // TODO find matching assets and decrement/delete course asset record
    const finalCourseAssets = {};
