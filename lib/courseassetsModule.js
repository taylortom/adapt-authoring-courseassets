...
class CourseAssetsModule extends AbstractApiModule {
  ...
  */
  ...
  async handleContentEvent(action, arg1, arg2) {
    ...
    const contentId = type !== "course" ? arg1._id.toString() : undefined;
    const courseId = type === "course" ? arg1._id.toString() : arg1._courseId.toString();
    const isModify = action === 'update';
    const logUpdate = () => this.log('debug', 'UPDATE', courseId, contentId);;
    // delete any existing course assets for content
    const deleted = await this.deleteMany({ courseId, contentId });

    if(action === 'delete') {
      if(deleted.length) logUpdate();
      return;
    }
